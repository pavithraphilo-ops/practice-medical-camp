...
def api_camp_patients(request, camp_id):
    ...
    # Gather all unique patient IDs from PatientVitals + PatientMedicineIssue + TestIssue for this camp
    vitals_pids = set(PatientVitals.objects.filter(camp=camp).values_list('patient_id', flat=True))
    issue_pids = set(PatientMedicineIssue.objects.filter(camp=camp).values_list('patient_id', flat=True))
